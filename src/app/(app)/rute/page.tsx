...
async function searchNominatim(query: string): Promise<GeoSuggestion[]> {
  try {
    ...
    const res = await fetch(`https://nominatim.openstreetmap.org/search?${params}`, { headers: { "User-Agent": "SiBanjir-Palembang/1.0" } })
    if (!res.ok) return []
...
function dedupeResults(results: GeoSuggestion[]): GeoSuggestion[] {
  ...
  for (const r of results) {
    const dup = out.some((o) => Math.abs(parseFloat(o.lat) - parseFloat(r.lat)) < 0.002 && Math.abs(parseFloat(o.lon) - parseFloat(r.lon)) < 0.002)
    if (!dup) out.push(r)
  }
  return out
}
...
async function geocodeMultiSource(query: string): Promise<GeoSuggestion[]> {
  ...
  const palembangQuery = baseQuery.toLowerCase().includes("palembang") ? baseQuery : `${baseQuery} Palembang`

  // Search in parallel: Nominatim (biased + unbiased) + Photon
  const [nominatimBiased, photonResults] = await Promise.all([searchNominatim(palembangQuery), searchPhoton(baseQuery)])
...
async function reverseGeocode(lat: number, lng: number): Promise<string> {
  try {
    const res = await fetch(`https://nominatim.openstreetmap.org/reverse?lat=${lat}&lon=${lng}&format=json`, { headers: { "User-Agent": "SiBanjir-Palembang/1.0" } })
    const data = await res.json()
...
export default function SafeRoutePage() {
  ...
  const [initialFloodMarkers, setInitialFloodMarkers] = useState<{ lat: number; lng: number; depth: string }[]>([])
  ...
  useEffect(() => {
    api
      .getMapReports(dateFrom || undefined, dateTo || undefined)
      .then((res) => {
  ...
  const searchRoute = useCallback(async () => {
    ...
    setRouteData(null)
    try {
      const res = await api.getSafeRoute(originCoords.lat, originCoords.lng, destCoords.lat, destCoords.lng)
  ...
  return (
    <div className="bg-surface text-on-surface antialiased h-screen overflow-hidden flex flex-col">
      <main className="relative flex-1 w-full mb-[72px] md:mb-0 flex flex-col bg-surface-container">
        {/* ── Toggle Button ───────────────────────────────────────────── */}
        <button onClick={() => setSidebarOpen((v) => !v)} className={`absolute z-30 flex items-center gap-1.5 bg-surface/90 backdrop-blur-md text-on-surface px-3 py-2.5 shadow-lg border border-outline-variant/50 hover:bg-surface-container-high transition-all duration-300 active:scale-95 ${sidebarOpen ? "top-3 right-3 rounded-lg md:top-4 md:left-[436px] md:right-auto md:rounded-r-xl md:rounded-l-none" : "top-3 left-3 rounded-lg"}`} title={sidebarOpen ? "Tutup panel" : "Buka panel navigasi"}>
          <span className="text-[20px] text-primary flex items-center justify-center">{sidebarOpen ? <X className="w-5 h-5" /> : <Navigation className="w-5 h-5" />}</span>
          {!sidebarOpen && <span className="text-label-bold font-bold text-[12px] tracking-wide hidden md:inline">NAVIGASI</span>}
        </button>

        {/* ── Left Panel (floating) ──────────────────────────────────── */}
        <section className={`z-20 w-full md:w-[420px] flex flex-col bg-surface/95 backdrop-blur-xl shadow-[0_8px_32px_rgba(0,0,0,0.18)] absolute top-0 left-0 h-auto max-h-[80vh] md:max-h-full md:h-full overflow-hidden rounded-b-2xl md:rounded-none md:rounded-r-2xl border-b md:border-b-0 md:border-r border-outline-variant/30 transition-all duration-300 ease-in-out ${sidebarOpen ? "translate-y-0 md:translate-x-0 opacity-100" : "-translate-y-full md:translate-y-0 md:-translate-x-full opacity-0 pointer-events-none"}`}>
          <div className="p-md flex flex-col gap-md flex-shrink-0 bg-surface z-10">
            ...
            <div className="flex items-center gap-1.5 overflow-x-auto hide-scrollbar -mt-1">
              ...
              <button onClick={() => setRange(today, today)} className={`whitespace-nowrap px-2.5 py-1 rounded-md text-[11px] font-semibold border transition-colors ${dateFrom === today && dateTo === today ? "bg-primary text-on-primary border-primary" : "bg-surface text-on-surface-variant border-outline-variant hover:bg-surface-container"}`}>
                Hari Ini
              </button>
              <button onClick={() => setRange(daysAgo(7), today)} className={`whitespace-nowrap px-2.5 py-1 rounded-md text-[11px] font-semibold border transition-colors ${dateFrom === daysAgo(7) && dateTo === today ? "bg-primary text-on-primary border-primary" : "bg-surface text-on-surface-variant border-outline-variant hover:bg-surface-container"}`}>
                7 Hari
              </button>
              ...
              {hasDateFilter && (
                <button
                  ...
                  className="p-1 rounded-md border border-outline-variant bg-surface text-error hover:bg-error-container transition-colors flex-shrink-0"
                  title="Reset filter"
                >
            ...
            <div className="bg-surface-container-low p-md rounded-xl flex flex-col gap-sm relative shadow-sm border border-outline-variant">
              ...
              <div className="relative">
                <div className="flex items-center gap-sm bg-surface p-sm rounded border border-outline-variant focus-within:border-primary focus-within:ring-1 ring-primary transition-all">
                  ...
                  <input type="text" placeholder="Masukkan lokasi awal..." value={originText} onChange={(e) => handleOriginChange(e.target.value)} onFocus={() => originSuggestions.length > 0 && setShowOriginList(true)} className="w-full bg-transparent border-none outline-none text-body-md text-on-surface placeholder:text-on-surface-variant/50" />
                  {originCoords && <CheckCircle className="w-5 h-5 text-green-600" />}
                </div>
                ...
                {showOriginList && originSuggestions.length > 0 && (
                  <div className="absolute top-full left-0 right-0 mt-1 bg-surface border border-outline-variant rounded-lg shadow-lg z-50 max-h-[200px] overflow-y-auto">
                    {originSuggestions.map((s, i) => (
                      <button key={i} onClick={() => selectOrigin(s)} className="w-full text-left px-3 py-2.5 text-body-sm hover:bg-surface-container-high transition-colors flex items-start gap-2 border-b border-outline-variant/30 last:border-b-0">
              ...
              {/* Swap button */}
              <button onClick={swapLocations} className="absolute right-md top-[50%] -translate-y-1/2 w-[32px] h-[32px] bg-surface rounded-full flex items-center justify-center border border-outline-variant hover:bg-surface-container-high transition-colors z-10 shadow-sm text-secondary active:scale-90">
              ...
              <div className="relative">
                <div className="flex items-center gap-sm bg-surface p-sm rounded border border-outline-variant focus-within:border-primary focus-within:ring-1 ring-primary transition-all z-10">
                  ...
                  <input type="text" placeholder="Masukkan tujuan..." value={destText} onChange={(e) => handleDestChange(e.target.value)} onFocus={() => destSuggestions.length > 0 && setShowDestList(true)} className="w-full bg-transparent border-none outline-none text-body-md text-on-surface placeholder:text-on-surface-variant/50" />
                  {destCoords && <CheckCircle className="w-5 h-5 text-green-600" />}
                </div>
                {/* Destination suggestions dropdown */}
                {showDestList && destSuggestions.length > 0 && (
                  <div className="absolute top-full left-0 right-0 mt-1 bg-surface border border-outline-variant rounded-lg shadow-lg z-50 max-h-[200px] overflow-y-auto">
                    {destSuggestions.map((s, i) => (
                      <button key={i} onClick={() => selectDest(s)} className="w-full text-left px-3 py-2.5 text-body-sm hover:bg-surface-container-high transition-colors flex items-start gap-2 border-b border-outline-variant/30 last:border-b-0">
            ...
            <div className="flex gap-sm">
              <button onClick={useMyLocation} disabled={isLocating} className="flex-1 flex items-center justify-center gap-xs bg-surface-container-high text-on-surface-variant text-body-sm font-medium py-2 px-3 rounded-lg border border-outline-variant hover:bg-surface-container transition-colors disabled:opacity-50">
                <span className="flex items-center justify-center">{isLocating ? <Hourglass className="w-4 h-4" /> : <Locate className="w-4 h-4" />}</span>
                {isLocating ? "Mencari..." : "Lokasi Saya"}
              </button>
              <button onClick={searchRoute} disabled={!canSearch} className="flex-[2] flex items-center justify-center gap-xs bg-primary text-on-primary text-body-sm font-semibold py-2 px-4 rounded-lg shadow-sm hover:bg-on-primary-fixed-variant transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
            ...
            {routeData && (
              <>
                ...
                {routeData.warningMessage && (
                  <div className={`p-sm rounded-xl flex items-start gap-sm text-body-sm font-medium border ${routeData.safeRoute.isSafe ? "bg-green-50 text-green-800 border-green-200" : routeData.safeRoute.isDetour ? "bg-amber-50 text-amber-800 border-amber-200" : "bg-red-50 text-red-800 border-red-200"}`}>
                  ...
                  <div className="bg-primary-container text-on-primary-container p-md rounded-xl border-2 border-primary cursor-pointer shadow-sm relative overflow-hidden flex flex-col gap-xs transition-transform active:scale-[0.98]">
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex flex-col gap-[2px]">
                        ...
                        <span className="text-h2 font-bold mt-xs text-on-primary-container">{formatDuration(safe.duration)}</span>
                      </div>
                      <span className="text-body-md font-medium">{formatDistance(safe.distance)}</span>
                    </div>
                    {safe.summary && <p className="text-body-sm opacity-90">Via {safe.summary}</p>}
                  ...
                  <div className="bg-error-container text-on-error-container p-md rounded-xl border border-error/30 cursor-pointer shadow-sm relative overflow-hidden flex flex-col gap-xs opacity-90 hover:opacity-100 transition-opacity">
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex flex-col">
                        ...
                        <span className="text-h2 font-bold mt-xs text-on-error-container">{formatDuration(flooded.duration)}</span>
                      </div>
                      <span className="text-body-md font-medium text-error">{formatDistance(flooded.distance)}</span>
                    </div>
                    {flooded.summary && <p className="text-body-sm mt-xs opacity-90">Via {flooded.summary}</p>}
                    <div className="mt-sm flex gap-sm flex-wrap">
                      {flooded.nearbyFloods?.slice(0, 2).map((f, i) => (
                        <span key={i} className="bg-error/10 text-error px-2 py-1 rounded-lg text-label-bold font-bold text-[10px] flex items-center gap-[4px] border border-error/20">
                ...
                {/* ── Navigate button ──────────────────────────────────── */}
                <button onClick={startNavigation} className="w-full mt-sm bg-primary text-on-primary text-h3 font-semibold py-sm rounded-full shadow-md flex items-center justify-center gap-sm hover:bg-on-primary-fixed-variant transition-colors active:scale-[0.97]">
                  <Navigation className="w-5 h-5" />
                  Mulai Navigasi
                </button>

                {/* ── Flood reports info ────────────────────────────────── */}
                {routeData.totalFloodReports > 0 && <p className="text-body-sm text-on-surface-variant text-center opacity-70">Berdasarkan {routeData.totalFloodReports} laporan banjir aktif</p>}
              </>
            )}
        ...
        <section className="absolute inset-0 w-full h-full bg-surface-container-highest overflow-hidden">
          <SafeRouteMapClient origin={originCoords} destination={destCoords} safeRoute={routeData?.safeRoute?.points} floodedRoute={routeData?.floodedRoute?.points} floodMarkers={routeData?.floodMarkers ?? initialFloodMarkers} isLoading={isLoading} />
        </section>
      </main>
