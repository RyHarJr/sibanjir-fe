...
const ICON_COLOR: Record<string, string> = {
  active: "text-error",
  surging: "text-warning",
  receded: "text-secondary",
}
...
export default function FeedLaporan() {
  ...
  const load = useCallback((s: SortType, from?: string, to?: string) => {
    setLoading(true)
    api
      .getReports({ sort: s, limit: 20, dateFrom: from || undefined, dateTo: to || undefined })
    ...
    <div className="bg-background text-on-background min-h-screen pb-20">
        ...
        <div className="max-w-3xl mx-auto flex items-center justify-between gap-3">
          ...
          <div className="flex gap-2 overflow-x-auto hide-scrollbar flex-1 min-w-0">
            {SORT_OPTIONS.map((opt) => (
              <button key={opt.key} onClick={() => setSort(opt.key)} className={`whitespace-nowrap px-3 py-1.5 md:px-4 md:py-2 rounded-full text-[12px] md:text-[14px] font-bold transition-colors ${sort === opt.key ? "bg-primary text-on-primary shadow-sm" : "bg-surface text-on-surface border border-outline-variant hover:bg-surface-variant"}`}>
                {opt.label}
              </button>
            ))}
          </div>
          ...
          <Popover className="relative shrink-0 flex items-center">
            {({ open }) => (
              <>
                <PopoverButton className={`p-2 rounded-full flex items-center justify-center transition-all outline-none focus:outline-none relative ${hasDateFilter || open ? "bg-primary-container text-on-primary-container shadow-sm" : "bg-surface border border-outline-variant text-on-surface-variant hover:bg-surface-variant"}`} title="Filter Tanggal">
      ...
      <main className="max-w-3xl mx-auto md:mt-4 md:px-gutter">
        <div className="flex flex-col gap-margin-mobile px-margin-mobile md:px-0">
          {loading ? (
            [1, 2, 3].map((i) => <div key={i} className="bg-surface rounded-xl h-64 animate-pulse border border-surface-variant" />)
          ) : feed.length === 0 ? (
                ...
                <article key={item.id} className="bg-surface rounded-xl shadow-card overflow-hidden border border-surface-variant relative">
                  ...
                  <div className="pl-3 pr-4 py-4 flex flex-col gap-3">
                    ...
                    {displayPhoto ? (
                      <div className="rounded-lg overflow-hidden relative mt-1 aspect-video bg-surface-container-highest">
                        ...
                        <div className="absolute bottom-2 left-2 bg-inverse-surface/80 text-inverse-on-surface px-2 py-1 rounded backdrop-blur-sm text-label-bold font-bold flex items-center gap-1">
                          ...
                          {(item._count?.photos ?? 0) > 1 && <span className="ml-1 opacity-80">+{item._count!.photos - 1} foto</span>}
                        </div>
                      </div>
                    ) : (
                    ...
                    <div className="flex items-center gap-2 pl-2 mt-1">
                      <span className="text-body-sm text-on-surface-variant">{confirms > 0 ? `Dikonfirmasi ${confirms} warga` : "Belum ada konfirmasi"}</span>
                    </div>

                    {/* Actions */}
                    <div className="flex flex-wrap sm:flex-nowrap gap-2 mt-2 pt-3 border-t border-surface-variant pl-2">
                      <button onClick={() => handleVote(item.id, "confirm")} disabled={voting[item.id]} className="flex-1 min-w-[120px] bg-primary text-on-primary text-[11px] sm:text-xs font-bold py-2 px-2 sm:px-3 rounded flex items-center justify-center gap-1 hover:bg-on-primary-fixed-variant transition-colors disabled:opacity-60">
                        <ThumbsUp className="w-4 h-4 md:w-5 md:h-5" />
                        Konfirmasi
                      </button>
                      <Link href={`/laporan/${item.id}`} className="flex-1 min-w-[100px] bg-secondary-container text-on-secondary-container text-[11px] sm:text-xs font-bold py-2 px-2 sm:px-3 rounded flex items-center justify-center gap-1 hover:bg-secondary-fixed-dim transition-colors">
                        <ExternalLink className="w-4 h-4 md:w-5 md:h-5" />
                        Detail
                      </Link>
                      <button onClick={() => handleVote(item.id, "reject")} disabled={voting[item.id]} className="bg-surface text-on-surface-variant border border-outline-variant text-[11px] sm:text-xs font-bold py-2 px-3 rounded flex items-center justify-center hover:bg-surface-variant transition-colors disabled:opacity-60 shrink-0" title="Laporkan tidak akurat">
