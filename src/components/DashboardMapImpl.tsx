...
function buildPinHtml(color: string, iconType: "alert" | "droplet", isCritical: boolean): string {
  const iconStr = iconType === "alert" ? `<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/><path d="M12 9v4"/><path d="M12 17h.01"/></svg>` : `<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22a7 7 0 0 0 7-7c0-2-1-3.9-3-5.5s-3.5-4-4-6.5c-.5 2.5-2 4.9-4 6.5C6 11.1 5 13 5 15a7 7 0 0 0 7 7z"/></svg>`
...
export default function DashboardMapImpl({ dateFrom, dateTo }: DashboardMapProps) {
  ...
  const markersData = mapReports.map((r) => ({
    lat: r.latitude,
    lng: r.longitude,
              ...
              <Marker key={i} position={[m.lat, m.lng]} icon={customIcon}>
                <Popup>
                  <div style={{ fontFamily: "'Public Sans',sans-serif", minWidth: "190px", padding: "4px" }}>
                    <div style={{ fontSize: "10px", fontWeight: 700, letterSpacing: ".06em", color: color, textTransform: "uppercase", marginBottom: "4px" }}>{isCritical ? "⚠ KRITIS" : "⚠ WASPADA"}</div>
                    <div style={{ fontSize: "14px", fontWeight: 700, color: "#1a1b21", lineHeight: 1.3, marginBottom: "6px" }}>{m.title}</div>
                    <div style={{ fontSize: "12px", color: "#555", lineHeight: 1.6 }}>
                      🌊 Kedalaman: <b>{m.depth}</b>
                      <br />
                      📍 Area: {m.area}
                      {m.confirms > 0 ? (
                        <>
                          <br />✅ <span style={{ color: "#00236f", fontWeight: 700 }}>{m.confirms} konfirmasi</span>
                        </>
                      ) : (
                        ""
                      )}
